...
见 [Pipeline F Dev Handback](./dev_handback.md#角色personal-与-asp-共用)。人测 gate 只认 **GitHub 负责人** 发的评论，不要求飞书提需人有 GitHub 账号。
...
D 已实现并开了 PR，尚未合 dev。E 用**与 D 不同的 Agent 平台**（如 D=Cursor、E=Claude Code；OpenCode 为 E fallback）读 PR diff + Analysis，判断：
...
- [Feishu Inbound Pipeline（A→F 总览）](./pipeline.md)
- [Pipeline F Dev Handback](./dev_handback.md)
- [Pipeline D Executor](./executor.md)
