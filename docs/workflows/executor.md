...
8. **禁止** `git add -f`；`contexts/life_record/profile/**` 只落本机（pre-commit hook：`bash tools/githooks/install.sh`）

---

## 相关

- [**Feishu Inbound Pipeline（顶层编排）**](./pipeline.md) — 全流程总览 A→F
- [Stage E: Gate Review](./gate_review.md)
- [Stage F: Dev Handback](./dev_handback.md)
- [Stage C: Feishu Inbound Agent](./agent.md) — 深度分析（D 的上游）
