...
- **边界**: 不 merge PR、不改代码；只读 GitHub Actions 结论 + 改 assignee + 发 comment；**不在此阶段处理业务验收结论**（业务验收由负责人在 E 人测 gate 代录，见 [Pipeline E Gate Review](./gate_review.md)）
...
本段与 [Pipeline E](./gate_review.md) 共用同一套名词，全文不混用英文别名。
...
- [Pipeline E Gate Review（含人测 gate）](./gate_review.md)
