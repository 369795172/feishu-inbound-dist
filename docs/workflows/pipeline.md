...
详见 [Pipeline F](./dev_handback.md#角色personal-与-asp-共用) 与 [Pipeline E](./gate_review.md#角色与-pipeline-f-共用)。
...
**Agent 单跳契约**: [../../skills/skill_feishu_inbound.md](./../../skills/skill_feishu_inbound.md)（`tools/grapeot_skills_sync.py` vendor，与 grapeot skills 同 pipeline）
...
**详细 workflow**：[triage.md](./triage.md)
...
**详细 workflow**：[executor.md](./executor.md)
...
**详细 workflow**：[gate_review.md](./gate_review.md)（AI 门禁）。存量人测 gate：[workflow_feishu_inbound_human_gate.md](./workflow_feishu_inbound_human_gate.md)。新 dev 验收：[acceptance.md](./acceptance.md)（引擎 [#35](https://github.com/369795172/feishu-inbound-skill/issues/35)）。
...
**详细 workflow**：[dev_handback.md](./dev_handback.md)
...
**Pipeline C 失败推进**（engine v0.1.33+，config `pipeline_c`）：单条分析失败不得永久堵死队列。

| 配置项 | 默认 | 含义 |
|--------|------|------|
| `batch` | 3 | 每轮 lead-tick / scan 最多分析条数（公平排序后截断） |
| `max_attempts` | 2 | 连续分析失败次数；达上限进入软冷却 |
| `cooldown_minutes` | 60 | 冷却期内跳过该 issue，冷却后自动再入队 |

失败计数写在 `issue_scanner_state.json`（`analysis_failure_count` / `last_analysis_failed_at`）。排序：失败少的优先，同失败次数仍按 `createdAt` 新优先。`--force` / `request-reanalysis` 绕过冷却。比 D 的 `execution-exhausted` 更轻，默认不要求人工摘 label。
...
- [Stage B 详细 workflow](./triage.md)
- [Stage C 详细 workflow](./agent.md)
- [Stage D 详细 workflow](./executor.md)
- [Stage E Gate Review](./gate_review.md)
- [Stage F Dev Handback](./dev_handback.md)
